...
export const tour = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'brochureUrl',
      title: 'Original brochure link',
      type: 'url',
      group: 'content',
      description: 'Optional downloadable brochure. Dates, prices and inclusions on older brochures must still be reconfirmed.',
    }),
    defineField({ name: 'overview', type: 'richText', group: 'content' }),
...
export const page = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'landingEyebrow',
      title: 'Landing section label',
      type: 'string',
      group: 'content',
      description: 'Small label above the restored overview section.',
    }),
    defineField({ name: 'landingTitle', title: 'Landing section title', type: 'string', group: 'content' }),
    defineField({
      name: 'landingIntro',
      title: 'Landing introduction paragraphs',
      type: 'array',
      of: [{ type: 'text', rows: 3 }],
      group: 'content',
    }),
    defineField({
      ...
      title: 'Landing information cards',
      type: 'array',
      group: 'content',
      of: [{
        type: 'object',
    ...
    defineField({
      name: 'groupCampusOptions',
      title: 'Additional group summer campus options',
      type: 'array',
      of: [{ type: 'string' }],
      group: 'content',
      description: 'Enquiry-led campuses that do not yet have a full programme page.',
      hidden: ({ document }) => document?.pageKey !== 'summerGroup',
    }),
    defineField({ name: 'faqs', type: 'array', of: [{ type: 'faqItem' }], group: 'details' }),
    ...closing,
  ],

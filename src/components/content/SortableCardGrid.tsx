...
const COPY = {
  en: {
    ...
    popular: 'Popular',
    az: 'A–Z',
    note: 'Curated starting order — not a live ranking.',
    explore: 'Explore',
  },
  tr: {
    sort: 'Sırala',
    popular: 'Popüler',
    az: 'A–Z',
    note: 'Editoryal başlangıç sırası — canlı bir sıralama değildir.',
    explore: 'Keşfet',
  },
} as const
...
export function SortableCardGrid({ locale, items }: { locale: Locale; items: SortableCardItem[] }) {
  ...
  const sorted = useMemo(() => {
    const next = [...items]
    if (sortMode === 'az') {
      next.sort((a, b) => a.title.localeCompare(b.title, language, { sensitivity: 'base' }))
    }
    return next
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-5 sm:p-6">
                ...
                <span aria-hidden="true" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-brand-strong">
                  {copy.explore} <span className="transition-transform duration-200 group-hover:translate-x-1">→</span>
                </span>
              </div>
            </Link>
          </li>
